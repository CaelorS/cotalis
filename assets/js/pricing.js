...
(function () {
  ...
  C.applyPricing = function (items, settings) {
    (items || []).sort((a, b) => (a.sort_order || 0) - (b.sort_order || 0)).forEach(row => {
      ...
      if (row.qty_mode) { it.qm = row.qty_mode; it.qc = row.qty_coef == null ? 1 : +row.qty_coef; it.qty = qtyFn(it.qm, it.qc); }
      if (it.custom) it.presets = row.presets || [];
    ...
    (settings || []).forEach(row => {
      ...
      if (row.key === 'PRESET' && v && typeof v === 'object') Object.keys(v).forEach(k => { if (Array.isArray(v[k])) C.PRESET[k] = v[k].slice(); });
      if (row.key === 'RULES' && Array.isArray(v)) { C.RULES.length = 0; v.forEach(r => C.RULES.push(r)); }
    });
  };
